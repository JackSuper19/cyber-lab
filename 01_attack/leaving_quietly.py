...
def leaving_quietly():
    ...
    arp_response.pdst = "192.168.236.2"  # Router's IP address
    
    # Set the MAC address of the router as the target hardware address
    arp_response.hwdst = "00:50:56:eb:f8:b4"  # Router's MAC address
    
    # Set the real MAC address of the victim (so the router learns the correct binding)
    arp_response.hwsrc = "00:0c:29:4a:a2:51"  # Victim's MAC address
    
    # Set the victim's IP address as the source IP
    arp_response.psrc = "192.168.236.132"  # Victim's IP address
    ...
    arp_response.pdst = "192.168.236.132"  # Victim's IP address
    
    # Set the victim's MAC address as the destination hardware
    arp_response.hwdst = "00:0c:29:4a:a2:51"  # Victim's MAC address
    
    # Set the real MAC address of the router (to reestablish trust)
    arp_response.hwsrc = "00:50:56:eb:f8:b4"  # Router's MAC address
    
    # Set the router's IP address as the source IP
    arp_response.psrc = "192.168.236.2"  # Router's IP address
